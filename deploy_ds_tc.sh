...
for i in "${SCRIPT[@]}"
do
    yarn oraicli wasm deploy $DIR_PATH/$i/artifacts/$i.wasm --label "$i $NONCE" --input $INPUT --fees $FEES
    ...
    then
        yarn oraicli provider set-$TYPE $i "test $i" $address --fees $FEES
    else
        # if it exists already, we update the contract
        yarn oraicli provider edit-$TYPE $i $i "test edit $i" $address --fees $FEES
    fi
    sleep 6
done

# ./deploy_ds_tc.sh datasource classification '' 17 ../oraiwasm/smart-contracts/package/cv 5000
# ./deploy_ds_tc.sh testcase classification_testcase '' 17 ../oraiwasm/smart-contracts/package/cv 5000